...
export const routePatterns = {
  creator: /^\/creator\/([^/]+)$/,
  content: /^\/content\/([^/]+)$/,
  contentEdit: /^\/mypage\/content\/edit\/([^/]+)$/,
};
...
export function matchRoute(fullPath) {
  const { creator, content, contentEdit } = routePatterns;
  ...
  if (creatorMatch) return { route: 'creatorProfile', params: { id: creatorMatch[1] } };
  
  const contentMatch = path.match(content);
  if (contentMatch) return { route: 'contentDetail', params: { id: contentMatch[1] } };
  
  const contentEditMatch = path.match(contentEdit);
  if (contentEditMatch) return { route: 'contentEdit', params: { id: contentEditMatch[1] } };
