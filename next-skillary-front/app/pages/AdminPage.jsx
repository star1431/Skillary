...
export function AdminPage({ onNavigate }) {
  ...
  const pendingCreators = mockCreators.filter((c) => c.status === 'PENDING');
  const approvedCreators = mockCreators.filter((c) => c.status === 'APPROVED');
  const rejectedCreators = mockCreators.filter((c) => c.status === 'REJECTED');
