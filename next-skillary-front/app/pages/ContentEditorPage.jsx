...
import { mockContents, mockCreators, categories, addContent, updateContent } from '../utils/mockData';
...
export function ContentEditorPage({ contentId, onNavigate }) {
  const { user } = useAuth();
  const existingContent = contentId ? mockContents.find((c) => c.id === contentId) : null;
  ...
  let creatorProfile = mockCreators.find((c) => c.userId === user.id);
  if (!creatorProfile) {
    // 임시 크리에이터 프로필 생성
    creatorProfile = {
    ...
    // mockCreators에 추가 (실제로는 API 호출 필요)
    mockCreators.push(creatorProfile);
  }
                ...
                <div className="px-6 pt-4">
                  <Tabs value={editorTab} onValueChange={setEditorTab}>
                    <div className="flex items-center justify-between gap-4 mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Button type="button" variant="outline" size="sm" onClick={() => { setMediaTab('image'); setShowMediaDialog(true); }}>
                          <ImagePlus className="mr-2 h-4 w-4" />
                          이미지
                        </Button>
                        <Button type="button" variant="outline" size="sm" onClick={() => { setMediaTab('video'); setShowMediaDialog(true); }}>
