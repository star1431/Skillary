...
import { mockSubscriptions, mockPurchases, mockPayments, mockPlans, mockCreators, mockContents, mockPayouts, updateCreator } from '../utils/mockData';
...
export function MyPage({ onNavigate }) {
  ...
  const userSubscriptions = mockSubscriptions
    ...
    .map((sub) => ({
      ...sub,
      plan: mockPlans.find((p) => p.id === sub.planId),
      creator: mockCreators.find((c) => c.id === sub.creatorId),
    }));
  ...
  const userPurchases = mockPurchases
    ...
    .map((purchase) => ({
      ...purchase,
      content: mockContents.find((c) => c.id === purchase.contentId),
    }))
  ...
  const creatorProfile = mockCreators.find((c) => c.userId === user.id);
  const creatorContents = creatorProfile
    ? mockContents.filter((c) => c.creatorId === creatorProfile.id)
    : [];
  ...
  const handleUpdateUser = async () => {
    ...
    if (user.role === 'CREATOR' && creatorProfile) {
      ...
      try {
        // 크리에이터 정보만 업데이트
        updateCreator(creatorProfile.id, {
