...
import { mockPlans, mockContents } from '../utils/mockData';
...
export function PaymentPage({ type, planId, contentId, onNavigate }) {
  ...
  const plan = type === 'subscription' && planId ? mockPlans.find((p) => p.id === planId) : null;
  const content =
    type === 'content' && contentId ? mockContents.find((c) => c.id === contentId) : null;
